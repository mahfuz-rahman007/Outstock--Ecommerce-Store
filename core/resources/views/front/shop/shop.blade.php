    ...
    @section('content')
        <main>
                                                                    ...
                                                                    <ul>
                                                                        @foreach ($category->productsubcategories as $psubcategory)
                                                                            <li><a class="{{ request()->input('psubcategory') == $psubcategory->slug ? 'active' : '' }}"
                                                                                    href="javascript:;"
                                                                                    data="{{ $category->slug . '_' . $psubcategory->slug }}"
                                                                                    id="category">{{ $psubcategory->name }}</a>
                                                                            </li>
                                                                        @endforeach
                                                                    </ul>
                                                                </div>
                        ...
                        <div class="col-xl-9 col-lg-9 col-md-8">
                            <div class="shop__content-area">
                                <div class="shop__header d-sm-flex justify-content-between align-items-center mb-40">
                                    <div class="shop__header-left">
                                        <div class="search-box">
                                            <input type="text" class="form-control px-4 py-4" id="searchProductInput"
                                    ...
                                    <div
                                        ...
                                        <div class="sort-wrapper">
                                            <select class="form-control" id="sortingProduct">
                                                <option value="new"
                                                    {{ request()->input('type') == 'new' ? 'selected' : '' }}>
                                                    {{ __('Newest') }}</option>
                                                <option value="old"
                                                    {{ request()->input('type') == 'old' ? 'selected' : '' }}>
                                                    {{ __('Oldest') }}</option>
                                                <option value="high_low"
                                                    {{ request()->input('type') == 'high_low' ? 'selected' : '' }}>
                                                    {{ __('Highest To Lowest') }}</option>
                                                <option value="low_high"
                                                    {{ request()->input('type') == 'low_high' ? 'selected' : '' }}>
                                                        ...
                                                        <div class="product__wrapper mb-60">
                                                                    ...
                                                                    @if (Auth::user())
                                                                        @if (Helper::isWishlist($product->id))
                                                                            <a href="javascript:;" class="active"
                                                                                id="remove_wishlist"
                                                                                data-add="{{ route('front.product.add.wishlist', $product->slug) }}"
                                                                                data-remove="{{ route('front.product.remove.wishlist', $product->slug) }}"
                                                                                data-href="{{ route('front.product.remove.wishlist', $product->slug) }}"
                                                                        ...
                                                                        @else
                                                                            <a href="javascript:;"
                                                                                data-href="{{ route('front.product.add.wishlist', $product->slug) }}"
                                                                                data-remove="{{ route('front.product.remove.wishlist', $product->slug) }}"
                                                                                data-add="{{ route('front.product.add.wishlist', $product->slug) }}"
                                                                                id="add_wishlist" data-toggle="tooltip"
                                                                                data-placement="top"
                                                                        ...
                                                                        @endif
                                                                    @else
                                                                        <a href="{{ route('user.login') }}"data-toggle="tooltip"
                                                                            data-placement="top"
                                                                            title="{{ __('Add to Wishlist') }}">
                                                            ...
                                                            <div class="product__content p-relative">
                                                                <div class="product__content-inner">
                                                                    <h4><a
                                                                            href="{{ route('front.product_details', $product->slug) }}">{{ $product->title }}</a>
                                                                    </h4>
                                                                    <div class="product__price transition-3">
                                                                        <span
                                                                            class="">{{ Helper::showCurrencyPrice($product->current_price) }}</span>
                                                                        @if ($product->previous_price != '0')
                                                                            <span
                                                                ...
                                                                <div class="add-cart p-absolute transition-3">
                                                                    @if (Auth::user())
                                                                        <a href="javascript:;"
                                                                            data-href="{{ route('front.product.add_cart', $product->id) }}"
                                                                            id="add_cart">{{ __('+ Add to Cart') }}</a>
                                                                    @else
                                                                        <a
                                                                            href="{{ route('user.login') }}">{{ __('+ Add to Cart') }}</a>
                                                                    @endif
                                                                </div>
                                                            </div>
                                                        </div>
                                ...
                                <div class="row mt-40">
                                    <div class="col-xl-12">
                                        <div
                                            class="shop-pagination-wrapper d-md-flex justify-content-center align-items-center">
                                            {{ $products->links() }}
                                        </div>
                                    </div>
                                </div>
            ...
            <form action="{{ route('front.products') }}" method="GET" id="search_product">
                ...
                <input type="hidden" name="psubcategory" value="{{ request()->input('psubcategory') }}"
                    id="psubcategory">
    ...
    @section('scripts')
        <script>
            ...
            $("#slider-range").slider({
                ...
                min: 0,
                max: 1200,
                values: ['{{ request()->input('min') ? request()->input('min') : 0 }}',
                    '{{ request()->input('max') ? request()->input('max') : '' }}'
                ],
